...
<BODY><H3>ERROR</H3>
<B>%s</B><P>
<FORM><INPUT TYPE=button VALUE=Back ONCLICK='window.history.back()'></FROM>
</BODY></HTML>'''
...
<P>Click <A HREF='%s'>here</A> to edit you data again.
</BODY></HTML>
<A HREF='%s'>log</A>'''
...
def doResult(who,howmany):
    f=open('log.html','a')
    f.write('<B>Name:%s</B><P>' % who)
    f.write('<B>Friends:%s</B><P>' % howmany)
    f.write('-----------------------------<P>')
    f.close()
    newurl=url+'?action=reedit&person=%s&howmany=%s'%(quote_plus(who),howmany)
    print(header+reshtml % (who,who,howmany,newurl,logurl))
